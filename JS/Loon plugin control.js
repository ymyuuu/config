...
if(body == null){if(isSurgeiOS || isLanceX || isStashiOS || isEgern){
	...
	$notification.post("QX转换：未获取到body","请检查网络及节点是否畅通","认为是bug?点击通知反馈",{url:"https://t.me/ymyuuu"})
 $done({ response: { status: 404 ,body:{} } });}else{
    console.log("QX转换：未获取到body的链接为" + $request.url)
    $notification.post("QX转换：未获取到body","请检查网络及节点是否畅通","认为是bug?点击通知反馈","https://t.me/ymyuuu")

...
const shouldSkipCache = (request, url) => {
    ...
    if (url.protocol === 'chrome-extension:') {
        return { skip: true, reason: '浏览器扩展请求不缓存' };
    }

    // 跳过配置中指定的路径
    if (CACHE_CONFIG.skipPaths.some(path => url.pathname.includes(path))) {
        return { skip: true, reason: '路径在禁止缓存列表中' };
    }
    ...
    if ((cacheControl && cacheControl.includes('no-store')) || pragma === 'no-cache') {
        return { skip: true, reason: '请求头指定不进行缓存' };
    }

    // 跳过带有认证信息的请求
    if (request.headers.get('Authorization')) {
        return { skip: true, reason: '带有认证信息的请求不缓存' };
    }
...
self.addEventListener('fetch', (event) => {
    ...
    if (shouldCache) {
        ...
        log(`${logMessage} - 跳过缓存: 非必缓存路径且非GET请求`, LOG_COLORS.SKIP);
    }
});

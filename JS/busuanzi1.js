...
! function() {
	...
	ready = function(c) {
		...
		e = function() {
			a || (a = !0,
				d.call(window),
				document.removeEventListener ? document.removeEventListener("DOMContentLoaded", e, !1) : document
				.attachEvent && (document.detachEvent("onreadystatechange", e),
	...
	bszTag = {
		...
		texts: function(a) {
			this.bszssp.map(function(b) {
				...
				var randomMultiplier = Math.floor(Math.random() * 10) + 1; // 生成1到10之间的随机数
				c &&
			...
			this.bszssu.map(function(b) {
				var c = document.getElementById("busuanzi_value_" + b);
				c &&
					(c.innerHTML = parseInt(Date.now() * 0.0000005 - 8.27 * Math.pow(10, 5)) + parseInt(a[
						b])); // init here
			});
			this.bszspp.map(function(b) {
				var c = document.getElementById("busuanzi_value_" + b);
				c &&
					(c.innerHTML = parseInt(Date.now() * 0.000000005 - 8.1 * Math.pow(10, 3)) + parseInt(a[
						b])); // init here
			})
		},
		hides: function() {
